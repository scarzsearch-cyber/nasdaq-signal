...
화면은 B 만 쓴다(v168 이후 선택 UI 없음) — A 는 signal.json 의 구버전 호환 미러로만 남는다.
성과지표는 deploy/build_stats.py 가 미리 굳혀둔
...
DEFENSIVE = {
    ...
    # [v206] 화면 카드가 이 문구를 그대로 그린다(defHtml). 재조정 규약은 v117 하나뿐이다 —
    #   「진입일부터 30일마다 확인 · 확인일 +2일 안에」(signal.html D-day 카운터 · watchdog rebalance
    #   와 같은 규약). 종전 「월 1회 (5%p 이상 벌어졌을 때만)」은 v23 시절 문구라 같은 카드 안에서
    #   D-day 와 어긋났다(2026-09-05 전수 감사 D-6).
    "rebalance": "도피 진입일부터 30일마다 비중 확인 (확인일 +2일 안에 재조정)",
