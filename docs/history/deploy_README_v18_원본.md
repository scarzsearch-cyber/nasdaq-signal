# [보관] deploy/README.md 의 v18 시절 원본 (2026-09-05 전수 감사에서 이관)

> **이 문서는 현행이 아니다.** v18(2026-08-26 이전) 시절의 「폰에서 보기 + 종가 자동 갱신」
> 설치 안내를 그대로 옮긴 것이다. 아래 내용 중 다음은 **현행과 정면으로 다르다**:
> stooq 출처(→ 야후 2경로 + 네이버 + 캐시, v66·v137 가드) · 22:30 UTC 단일 슬롯(→ 마감 전
> 4 + 예비 4 = 8슬롯, v190) · 「두 전략을 화면에서 고른다」(→ v168 부터 B 만, A 는 참조 미러) ·
> `build_stats.py` 를 사람이 로컬에서 돌려 커밋(→ v171 월간 워크플로 자동). 현행 안내는
> `deploy/README.md`(재작성본) · `03_System_Params.md` §3 · `CLAUDE.md` §4 를 본다.
> 보관 이유: v175 규약(이력을 지우지 않는다) — 당시 설치 절차의 증거로만 남긴다.
