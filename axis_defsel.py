...
def s9_verdict():
    ...
    print('  · §1  도피 구간 연율: 배당 8.57 / 국채 7.92 / 금 8.55 인데 **바스켓은 9.32** 다.')
    print('    바스켓이 자기 성분 셋을 전부 이긴다. 이것이 재조정 프리미엄이고, 고르는')
    print('    순간 버리는 몫이다. 분산 자체가 이 축의 알파다.')
    print('  · §1  현금100 은 시험한 모든 정책 중 꼴찌다(-33%, 1972-). MDD 도 더 나쁘다')
    print('    (-63.1% vs -59.7%) — 위기에 버는 자산이 있으면 낙폭 자체가 얕아지기 때문이다.')
