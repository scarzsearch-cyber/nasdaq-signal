...
- `audit/test_account_ledger.py` — 새 계좌 원장의 손계산·독립 이분법 거래해·무작위 장부·휴일 현금·분배 중복 검산.
- `research/strategy_f1_placebo.py` — F1 비중 시점/원료 재계산의 연 블록 반증 999회와 임의 구간 5,000개. 다중보정·회계 축퇴·음성 대조군 검사. 채택 판정 아님.
...
- `audit/DEEP_REVIEW_OPS_UI_2026-09-05.md` — 운영·화면 20파일 2차 심층 코드리뷰 장부(v220): 워크플로 호출표·함수 단위 계약·데이터 계약·실행 검사·발견 R2-nn.
...
- `audit/A11Y_2026-09-06.md` — 설명서·노트 다크 모드 희귀 상태·라이트 점검 알약 대비 실측(v227): 수정 2(노트 켜진 칩 · 알약 라이트) · 라이트 토큰 제안.
- `audit/SURVIVAL_2026-09-06.md` — 자동화 생존·감시 실패 검증: 60일 규칙(공식 문서·실행 이력 분리 · 봇 커밋 효과 미확인) · 동시 정지 구조 · 토큰 만료/GH_PAT 부재/채널 부재 경고 경로 격리 검증.
- `audit/test_survival5.py` — 위 경고 경로 회귀 13(channel 5 · keepalive 5 · 워크플로 계약 3).
- `audit/GATES_DATA_2026-09-06.md` — 자료 의존 관문 I1·I5·I10 실패 탐지: 격리 클론 원자료 합성(완만 상승·횡보·톱니·환율 상수·금 붕괴) + I1 회계 결함 3 · 잡힘 9/사각지대 1(check_hold 다자산 재조정 · 인계).
...
- `audit/test_a11y12.py` — 위 계약 회귀 6(리퍼러 meta·noopener · live 영역·role · 되돌리기 초점 복귀 순서(v232) · 검색·필터 상태 · 접기 aria-expanded · 정적 컨트롤 이름) · **verify.yml unittest 목록 등재**(2026-09-06 소유자 지시 · 정적 검사라 브라우저 불요).
...
- `audit/screen_states.py` — 화면 상태 행렬 생성기: 상태별 폴더(signal.json·장부·시세·점검 합성)를 만들어 정적 서버로 연다. 표시 검사 전용 · 출력 폴더는 커밋하지 않는다.
- `audit/GATE_MUTATION_2026-09-05.md` — verify_all 관문 변별력 검사 장부(5차): 변조 127개 전부 잡힘 · 부분문자열 검사 4곳 강화(옛 verify_all 로 재현).
- `audit/gate_mutation.py` — 관문 변별력 도구: 임시 클론(작업본 반영)에 관문마다 결함을 주입해 FAIL/WARN 여부를 본다(`--only`·`--verify 옛사본`·`--list`). 예약·수동 검증에서만 돈다.
- `audit/WATCHDOG_CHAIN_2026-09-05.md` — 파수꾼 연결 검증 장부(v222 후속): check 연쇄·주간 커밋 셸·워크플로 계약·토큰 회전·heartbeat 유실 · 규약 인계 4건.
- `audit/test_watchdog_chain4.py` — 파수꾼 연결 회귀 16검사: 평가기 출력 계약(실제 문자열 6경우)·주간 check 연쇄·커밋 셸(bare 원격)·워크플로 id/if/이슈 조건·토큰 회전 연속성·heartbeat 커밋 유실.
- `audit/test_ops_recovery3.py` — v222 통합 회귀 28검사: 신호 JSON 원자 교체·예상일 조회 실패 시 확정 갱신 인정·**NYSE 달력 기준 최신성(원천 전체 지연·주말·휴장·장중 누락, 생산자 파서→대기 루프 연결)**·알림 부분 실패 재발송·시세 조용한 장 재발행·월간 부분 실패 종료코드·일일 커밋 셸의 push 경쟁(로컬 bare 원격).
...
| `research/ml_policy.py` | **[새 길 탐구 · 가상] 기계가 배우는 정책** — 특징 15 · 걸어가며 재학습 로지스틱(뉴턴·L2) · 매년 570개 한 조건 규칙 탐색(규칙 재발견 검사) · OOS 1982~ | research/EXPLORATION.md §B · MDD −91~−99% · 낙폭 계열 재발견 3/45년(v210 재실행 14/45 · 결론 동일) — 자유도의 가격 |
