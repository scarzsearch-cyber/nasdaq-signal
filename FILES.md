...
docs/       (43) 전략_v*.md · raw/ · HANDOFF_전체이력
...
| 파일 | 역할 |
|---|---|
| `docs/전략_v64.md` | **최신.** 지표 풀이 네 줄 + 값에 강조색 · MDD 방향 표시 정정. **규칙 변경 없음** |
| `docs/전략_v63.md` | 같은 기간으로 맞춘 표 — 최근 5·10·15년은 2배 보유 승, 20년에서 뒤집힘 |
| `docs/전략_v62.md` | 지표를 「그래서 얼마냐」로 번역 + 체결 시각 모순 정정(v18 잔재) |
| `docs/전략_v61.md` | A(−16/−11) 화면 제거 + 지표에 눈금(2배 보유·방어 단독) |
| `docs/전략_v60.md` | 165 vs 2,761 분해(전부 그 3년) + 회복기간·Ulcer 화면 반영 |
...
| `docs/전략_v32.md` | 변동성 조기방어 재심 — v31 기각근거 2개는 틀렸지만 **적립식에서 재기각**. 전략 변경 없음 |
| `docs/전략_v31.md` | v30 감사(근거 3개 정정) + 금리차·복귀필터·변동성 조기방어 기각. 전략 변경 없음 |
| `docs/전략_v30.md` | 매크로·심리 지표 전부 기각 — 동행지표라 새 정보가 없다. **v31 로 정정됨** |
| `docs/전략_v29.md` | ISA 서민형 세후 — 일반계좌 대비 +48.8%, 이득의 80%는 과세이연. 화면 반영 |
| `docs/전략_v28.md` | 신호원은 미국 QQQ 종가 — 국내 종가로 재면 1997- −92%, MDD +21%p |
| `docs/전략_v27.md` | 방어 안에서의 선택 기각 + 월간 재조정 비용 버그 정정. 분산 자체가 알파 |
| `docs/전략_v26.md` | 적립식 재검증 — 불규칙 납입도 순위 불변. 실무 최소 월납입 15만원 |
| `docs/전략_v25.md` | 남은 과제 3건 완결 — 실측 괴리율·레버리지 검증·제헌절. **v24 §3.3/§3.4 정정** |
| `docs/전략_v24.md` | 상품 사양 실측·공휴일. §3.3/§3.4 는 v25 에서 정정됨 |
| `docs/전략_v23.md` | **채택안.** 방어자산 = 배당40 / 미국채40 / 금20 (전부 국내 상장·환노출) |
| `docs/전략_v22.md` | 새 축 3개 판정 — 배수(조건부) / 앙상블(기각) / 적립식(현행 강화) |
| `docs/전략_v21.md` | **현행 채택안의 근거.** 방어자산 실측화 + 한국 실전 + 3자 비교 |
| `docs/전략_v20.md` | 54년 확장·히스테리시스. §11.11 이 인용하는 유의성 통계의 출처 |
| `docs/전략_v19.md` | 복귀로직 294+576개 전부 기각 기록 |
| `docs/전략_v18.md` | 진입선 45개 변형 기각 + 기본 규약 정의 |
