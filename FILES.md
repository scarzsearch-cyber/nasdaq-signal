...
| 파일 | 역할 |
|---|---|
| `전략_v36.md` | **최신.** 국채 다리 선물형 정정 — 드리프트 +2.84%→−0.18%. 채택 결정 유지, 공표수치 −18.6% |
| `전략_v35.md` | 전수조사(59파일, HIGH 0건 · 시점별 재계산 0/40) + 라이브 종가 미갱신 수정 |
| `전략_v34.md` | 전면 감사 — 채택 결정 전부 유지, 실패 0건. 남은 경고는 국채 롤 비용 1건 |
| `전략_v33.md` | 적립 시뮬레이터 2일 지연 버그 정정 — v29 ISA +43.2%→+49.7%. 결론 불변 |
| `전략_v32.md` | 변동성 조기방어 재심 — v31 기각근거 2개는 틀렸지만 **적립식에서 재기각**. 전략 변경 없음 |
| `전략_v31.md` | v30 감사(근거 3개 정정) + 금리차·복귀필터·변동성 조기방어 기각. 전략 변경 없음 |
| `전략_v30.md` | 매크로·심리 지표 전부 기각 — 동행지표라 새 정보가 없다. **v31 로 정정됨** |
| `전략_v29.md` | ISA 서민형 세후 — 일반계좌 대비 +43.2%, 이득의 80%는 과세이연. 화면 반영 |
| `전략_v28.md` | 신호원은 미국 QQQ 종가 — 국내 종가로 재면 1997- −92%, MDD +21%p |
