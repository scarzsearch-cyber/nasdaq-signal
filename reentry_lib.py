...
[start= 의 뜻 — B03-3 · 2026-09-05]  run(start=) 은 시작일에 상태기계를 w0(기본 1=공격)로
**다시 시작**한다(시작일 이전 신호를 모른다). axis_lib.sim(start=) 은 전체 경로를 자를 뿐이다.
시작일이 방어 구간이면 run 은 첫날 1→0 유령 전환 비용(편도 0.1%)을 한 번 더 물고, A 처럼
히스테리시스 띠가 있으면 띠 안 시작일에서 상태가 갈린다. 실측(B): run/sim = 0.9990 (2008-12-01
· 2002-10-01 · 2022-06-01 시작). 공표 4시나리오(1972-02-07 · 2000-01-03 · 1997-01-02 · 2023-06-20)는
전부 공격 시작이라 무관하다. run() 은 verify_all I8 봉인이라 이 주석은 함수 밖에 둔다.
"""
import numpy as np
import pandas as pd
