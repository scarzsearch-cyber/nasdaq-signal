...
| 방어 | 458730 40% / 305080 40% / 411060 20% · 재조정은 **도피 진입일부터 30일마다 비중 확인**(운영 규약 v117 · signal.json `defensive.rebalance` 와 동일). ⚠ freeze.json 에는 재조정 주기 항목이 없고, 백테스트 모형(`mix_monthly_from`)은 **월말 재조정**이다 — 둘은 같은 값이 아니다 |
...
```
daily-signal.yml   [v75] 대기 루프 체제 — 트리거 8개([v190] 04:35/04:45/05:35/05:45 + 05:17/06:17/07:47/09:17 KST) 중
                   **하나만 떠도** wait_close.py 가 종가 반영까지 240초 간격 재시도(최대 170분).
                   [v190] 마감 전에 뜬 실행은 마감까지 자고 20초 간격으로 종가가 굳는 순간을 잡는다 → 마감 뒤 5분 안.
...
watchdog.yml         [v140] 자동 파수꾼 — **「실패」가 아니라 「아예 안 돈 것」을 잡는다**
  ...
  ① watchdog.py stale    평일 08:40 KST — signal.json as_of 가 **미국 거래일 3일**(주말·NYSE 정기 휴장 제외 · v225) 밀리면 카톡
                         (3거래일마다 1회만 · 한국장 개장 20분 전 · 판정과 무관한 신선도 경고)
...
| I8 | 공용 모형 8종 **소스 봉인**(`SHARED_SEAL`) — 바뀌면 실패. 사용처를 재실행한 뒤 의도적으로 봉인을 갱신해야 통과 (2026-09-04 코드리뷰) |
...
docs/history/         ← 전략_v18~v203 보관 (60개, 온디맨드 참조. v80 §6·§7 = T4 판정 부속서)
