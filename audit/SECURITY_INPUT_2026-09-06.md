...
## 4. 미확인
- 실제 Actions 로그에서 `::add-mask::` 가 적용된 모습은 **다음 실제 토큰 회전 때**만 확인된다(카카오가 새 refresh 토큰을 줄 때 · 시점 불명). 격리 검사는 stdout 명령 문자열까지다.
- 브라우저 CSP 는 없다(세 화면) — 같은 오리진 XSS 가 생기면 localStorage 장부 전체가 노출되는 구조는 그대로. 이번 범위에서 외부 입력이 innerHTML 에 닿는 경로는 못 찾았지만 「없음」의 증명은 아니다.
- 카톡 도착·실기기는 기존대로 미확인.
