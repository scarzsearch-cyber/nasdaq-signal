...
| A-1 | P2 | 소유자 아티팩트 「전략 B 배합 탐색기」 배율 탭 (https://claude.ai/code/artifact/8b1e7810-c3e8-46f5-a2fe-b4b6cfe5a79d) | 「읽는 법」이 v203 교정 이전 값(146.1 vs 146.6 동률 · 손익분기 2.5~2.7 · 0.91배 · 62.8 vs 55.7 · 「세금 격차는 버블 탓 아님」 · 「환 효과 없음」)을 그대로 실음 — 결론 박스(원화 v2)와 한 화면 안에서 모순 | **완료** — 2026-09-05 재발행(v3 라벨). 배율 탭 LEV 를 `tax_us_direct.py --emit/--c21/--windows/--accum` 출력으로 재조립 · 결론·읽는 법·세금/손익분기 문구를 데이터 기반으로 다시 씀 · node 구문검사+스모크(탭·배율·지평·납입 전환) 통과 |
...
| A-2 | P3 | 아티팩트 배합 탭 `DATA.rows` | 생성 스크립트가 저장소에 없음(재현 불가 표) | **표시 완료** — 표를 지우지 않고 caveat·footer 에 「2026-09-03 계산분 · 재현 스크립트 없음」 명기. 재계산은 별도 과제 |
