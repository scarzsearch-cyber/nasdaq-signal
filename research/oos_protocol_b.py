...
사전 등록 (결과 보기 전에 적음 — §-1 ⑤ 「실패하면 뭐가 참 / 통과하면 뭐가 참」)
  ...
  A 재난 지급: H(2배 맨몸) 사건창 MDD ≤ −50% 인 사건에서 B 의 사건창 MDD 가 H 보다 얕다.
      실패 = 「느린 약세장 보험」 기전이 그 재난에서 작동하지 않았다 (역사 7/7 → 1회 실패 = 역사 밖).
      통과 = 보험이 지급됐다 — 규칙 유지의 근거.
  B 보험료 상한: 사건창 B/H 총수익비 − 1 이 역사 P05(−33.3%) 아래면 주의, 역사 최악(−41.1%) 아래면 역사 밖.
      실패 = 보험료가 역사보다 비싸다 — 기전 붕괴가 아니라 국면 변화일 수 있다 → 재검토 연구 개시.
  R  누적 출혈: 3년 롤링 B/H − 1 이 P05(−31.5%) 아래면 주의, 최악(−49.3%) 아래면 역사 밖. 상시.
...
def evaluate_oos(D):
    ...
    dis = pre[(pre.mdd_h <= -0.5) & pre.indep]              # A 의 등록값 7/7 은 **독립** 사건 기준
    a_pass, a_n = int((dis.mdd_b > dis.mdd_h).sum()), len(dis)
    p05 = pct(pre.prem, 5)                                    # B 의 P05 는 전체 사건(70) 기준
