...
[룰 준수 선언]
  ...
  · 이미 기각된 축은 재실행하지 않고 문서를 인용한다:
      히스테리시스·확인일·최소유지일(v82 K관문 9/9 전멸) · 무거래 밴드(v80 §4 당시 기각;
      2026-09-05 R06: 무거래 회계 오류 교정, 옛 값/불가능성 주장은 현행 근거로 사용 금지)
      · RV비율/변화율(v54 G1, 4블록 2/4 탈락) · RV수준 상태변수(v53 G11 탈락)
      · 복귀 최적화 단독(v41 조기복귀 기각·v59 최적복귀일=저점 포착 0 이하)
      · ¼ 양자화(v81 §3 당시 진단 — 목표변경일을 매매일로 오인하면 안 됨; R06 참조).
...
HALF = '2000-01-01'
V81 = dict(b1=170800, t1=163161, mix1=185582, b2=148455, t2=109451)   # v81/v83 공표 (v210 거래일 정정 전 · 역사 기록)
# [순회 B08 · 2026-09-05] v210(FRED 빈 가격 행 114개 제거) 뒤 **같은 코드**로 다시 재면 B +7.5% ·
#   T4 +57.7% · MIX25 +43% 가 되어 V81 앵커로는 0단계에서 중단됐다. v203·v209 작업트리는 V81 을
#   소수점까지 재현했으므로 움직인 원인은 자료(v210) 하나뿐이다(§-1 ⑧ — 한 열만 바꿔 확인).
#   재현 검산은 현행 자료 기준 V210 으로 하고 V81 은 참고로 함께 찍는다. 자료가 또 바뀌면 이 자리를
...
def main():
    ...
    ok_rep = True
    for nm, c, k in pairs:
        f = float(c.iloc[-1]); err = f / REF[k] - 1
        ok_rep &= abs(err) <= 0.015
        print('  %-12s %10s  (v210 기준 %s, 오차 %+.2f%% · v81/83 %s)'
