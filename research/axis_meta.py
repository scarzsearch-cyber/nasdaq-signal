...
L = 20 * 252
# [코드리뷰 2026-09-04] 마지막 블록은 4.7년짜리 토막인데 종전에는 「비중첩 10년창」
#   헤더 아래에서 나머지 넷과 동등한 한 표로 세어졌다(>=3/5 관문). 온전한 10년 블록과
#   구분해 표시하고, 블록 관문은 **온전한 4개만** 센다. 토막은 참고로 인쇄한다.
...
FULL_BLOCKS = 4          # 앞 4개만 온전한 10년. 마지막(*)은 4.7년 토막.
# =============================================================
...
def main():
    ...
    def sc_D(lo, hi, i):
        # ⚠ [코드리뷰 2026-09-04] 이 함수의 x0.9 는 **자료가 아니라 풀 기하구조**가 정한다.
        #   _neighbors 의 허용오차 0.011 로 이웃이 있는 것은 대칭 군집(-.10 -.11 -.12
        #   -.15 -.16)뿐이고, 비대칭 7개(-.16/-.11 · -.19/-.11 · -.19/-.18 · -.23/-.07
        #   · -.23/-.18 · -.24/-.22 · -.24/-.24)는 실제 강건성과 무관하게 **영원히** 벌점을
        #   진다. 벌점을 빼면 분기별 선택이 180분기 중 38분기(21%)에서 바뀐다.
        #   설계는 결과 보기 전에 동결됐으므로(문서 머리 §4) 계산은 그대로 두고,
        #   「메타 D 강건성」 행을 강건성의 측정으로 읽지 말라는 사실만 남긴다.
    ...
    print()
    print("  비중첩 10년창: %s" % ' '.join('%-6s' % b[0] for b in BLOCKS))
    print("  * 2022-26 은 4.7년 토막이다 — 표시만 하고 블록 관문(>=3)에는 안 센다.")
    print()
    ...
    for nm in cands:
        ...
        if (np.median(r['isa']) > np.median(B['isa'])
                ...
                and sum(1 for v, b in zip(r['blk'][:FULL_BLOCKS],
                                          B['blk'][:FULL_BLOCKS]) if v > b) >= 3
                and r['mdd'] >= B['mdd']):
            win.append(nm)
    print("=" * 118)
    # [코드리뷰 2026-09-04] Oracle 검사가 adopt_if 밖이라 O 든 X 든 판정이 같았다
    #   (2x2 를 전부 돌려 확인: win 만 결과를 정하고 Oracle 은 아무 영향이 없다).
    #   그런데 이 검사가 X 라면 뜻이 다르다 — 상한 자체가 없으므로 「기각」이 아니라
    #   **「판정 불가」**다. 상한이 있을 때만 기각/채택을 말하도록 순서를 바꾼다.
    if ob <= 0.10:
        print(verdict('동적 규칙 선택이 고정 -16/-16 을 이기는가 (§29)', [
            ('Oracle 이 고정을 유의하게 앞선다 (전환 가치가 애초에 있는가)',
             False, 'Oracle-고정 %+.0f%% — 상한이 없으므로 메타를 판정할 수 없다' % (ob * 100)),
        ])['text'])
        print('  -> 판정 불가: 사후에 최적 규칙을 골라도 고정을 못 이긴다. '
              '메타의 성패는 이 표본에서 의미가 없다.')
    else:
        print(verdict('동적 규칙 선택이 고정 -16/-16 을 이기는가 (§29)', [
            ('Oracle 이 고정을 유의하게 앞선다 (전환 가치가 애초에 있는가)',
             True, 'Oracle-고정 %+.0f%%' % (ob * 100)),
