...
def prep(px):
    """1배·2배 합성 일수익. 2배는 drag = DRAG_RATIO x 일간분산 을 매일 뺀다.

    [2026-09-04 코드리뷰] 종전 식은 `... * 252) / 252` 라 곱했다 나누는 무동작이
    끼어 있었다(값은 같다 — 읽는 사람만 연율화가 있는 줄 안다). 그리고 분산은
    **전 표본**에서 잡는다: 시점별로는 알 수 없는 값이라 여기 배수 열은
    point-in-time 수치가 아니다. aB·a2 양쪽에 같은 상수로 들어가 ①의 판정
    지표인 비율에는 안 걸리지만, 절대 배수를 인용할 땐 이 전제를 붙여라.
    """
    r1 = np.nan_to_num(px.pct_change().values)
    r2 = 2 * r1 - DRAG_RATIO * float(np.nanvar(r1))
    return r1, r2
...
def test1():
    ...
    print(' ⚠ 이 관문은 **철회됐다**(04 §5-13 정정1 · CLAUDE §-1 ⑤). 「적절한 특화」와')
    print(' 「과적합」이 같은 결과를 내므로 구분하지 못한다 — 아래는 서술 자료일 뿐')
    print(' 합격/불합격이 아니다.')
    print('=' * 76)
    ...
    for lab, path in MARKETS:
        ...
        if len(px) < 252 * 10:
            continue
        _, r2 = prep(px)      # [코드리뷰] 1배 계열은 여기서 안 쓴다
        ...
        print(f'{lab:>10} {str(px.index[0].date())[:7]}~{str(px.index[-1].date())[:7]} '
              ...
              f'{"승" if win else "패":>5}')
    # [2026-09-04 코드리뷰] 종전엔 여기서 「★(과반 패 — 나스닥 전용 의심)」을 찍었다.
    # 그 판정은 04 §5-13 정정1 에서 철회됐는데 철회문은 100줄 아래 종합 판정에만
    # 있었다 — ★ 를 훑는 사람은 이미 철회된 과적합 신호를 주워 갔다(§-1 ③).
    print(f'\n  → {total}개 시장 중 {wins}승 {total-wins}패 (서술 자료 · 판정 아님)')
    return wins, total
...
def test2():
    ...
    print(' ⚠ 이 구현은 **결함 설계**다(04 §5-13 정정2): 문턱을 직전 창이 아니라')
    print(' 시작점부터의 **누적 성과**로 고른다. 아래 수치가 아니라 정정2 표를 볼 것')
    print(' — 직전 N년 창으로 재설계하면 선택 중앙이 −0.16 이다.')
    print('=' * 76)
...
def main():
    w, t = test1()
    test2()                       # [코드리뷰] 반환 판정은 안 쓴다 — 결함 설계라 판정 불가
    f3 = test3()
    print('\n' + '=' * 76)
    print(' 종합 판정')
    print('=' * 76)
    ...
    print('  ② 문턱 WFA     : **판정 없음 — 결함 설계**(04 §5-13 정정2)')
    print('     누적 성과로 문턱을 고르는 구현이라 이 스크립트의 ② 수치는 판정에 못 쓴다.')
    print('     직전 N년 창으로 재설계하면 선택 중앙이 −0.16 이고 −16 고정이 적응형을')
    print('     67~77% 이긴다. 정정2 표를 볼 것.')
