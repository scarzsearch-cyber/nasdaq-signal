...
def main():
    ...
    print(verdict('실현변동성 상태변수(T=%.2f)를 채택할 수 있는가' % T0, [
        ...
        ('G11 집중도 독립검사 통과', False,
         'axis_gate11.py: 2000년 두 구간이 우위의 87%, 상위 3개 제거 시 P5 역전'),
    ], adopt_if=indep)['text'])
    # G11은 별도 파일에서 계산하지만 최종 판정에서 빠지면 이 파일만 실행한 사람이
    # 이미 기각된 후보를 다시 「채택」으로 읽는다. 알려진 독립검사 결과를 관문에 포함한다.
