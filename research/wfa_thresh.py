...
★ 실행 결과 — **문서 표는 재현된다. 단 조건이 둘 있었고, 문서엔 안 적혀 있었다.**
  ...
  ⚠ [v210 자료 재실행 2026-09-05 · 순회 B15] ext 엔진 **7/12** — 선택 중앙 −0.16 은 3/3 그대로이고 어긋난 것은
     「고정 승(무승부 포함)」(73/77/67 → 79/92/75%)·걸음5y 의 밴드/정확히(걸음 8~14회라 1~2걸음 차이)다.
     ext 결함판(누적)의 선택 중앙은 −0.16 → −0.12 로 움직였다(정정판은 −0.16 유지 · 훈련 길이 8/8). v209 까지는 12/12.
...
def main():
    ...
    cm, wm = cell[('ext 1972~', 'cum')]['med'], cell[('ext 1972~', 'window')]['med']
    # [순회 B15] 문장을 값에 맞춘다 — 종전엔 「안 움직인다」가 하드코딩이라 v210 뒤 −0.12 → −0.16 인데도 그렇게 찍었다.
    print('     같은 엔진(ext)에서 창만 바꾸면 선택 중앙은 **%.2f -> %.2f** 로 **%s**'
          % (cm, wm, '안 움직인다.' if abs(cm - wm) < 0.005 else '움직인다 — 이 자료에서는 창 수정이 선택을 −16 쪽으로 옮긴다.'))
    print()
