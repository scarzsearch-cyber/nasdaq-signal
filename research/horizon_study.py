...
[지평 연구, 소유자 지시 2026-08-31] 「내 수명은 유한하다 — 3~20년만 연구하라」.
  ...
  A. 현행 B(2배, 실물 체인) — 지평 3/5/7/10/15/20년 창 전수:
     손실확률 · p05 · 중앙 · 최악 · P(B>맨몸2배) · P(B>헤지6/4 근사)
...
def main():
    ...
    print('\n[A] 현행 B(2배·실물 체인) — 지평별 창 전수 (54년 검증 표본)')
    print(f"{'지평':>5} {'창수':>6} {'손실확률':>8} {'p05':>6} {'중앙':>7} {'최악':>6} "
          f"{'P(B>맨몸)':>9} {'P(B>헤지)':>9}")
    for w, lab in HS:
        mb = aB[w:] / aB[:-w]
        mh = aH[w:] / aH[:-w]
        md = aHd[w:] / aHd[:-w]
        print(f'{lab:>5} {len(mb):>6} {np.mean(mb < 1):>8.1%} {np.quantile(mb, 0.05):>5.2f}배 '
